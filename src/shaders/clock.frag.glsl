...
vec4 clock(vec2 uv, vec4 color, float i, float cameraAngleCoef) {
  ...
  if (i < uCount) {
    float mixAmountCamera =
      1.0 - (1.0 - mixAmount) * pow(cameraAngleCoef, 0.75);

    strokeColor = mix(strokeColor, vec4(0.0), vec4(vec3(mixAmountCamera), 0.0));
    secondsStrokeColor = mix(
      secondsStrokeColor,
      vec4(0.0),
      vec4(vec3(mixAmountCamera), 0.0)
    );
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    color = stroke(
      sdSegment(
        uv,
        vec2(sin(angle), cos(angle)) * 0.85,
        vec2(sin(angle), cos(angle)) * 0.94
      ),
      color,
      strokeColor,
      sh,
  ...
  float hoursAngle =
    float(uHours) * (2.0 * PI) / 12.0 +
    float(uMinutes) * (2.0 * PI) / 12.0 / 60.0;
  color = stroke(
    opOnion(
      sdSegment(
        uv,
        vec2(0.0, 0.0),
        vec2(sin(hoursAngle), cos(hoursAngle)) * 0.5
      ),
      origTh / 3.0
    ),
    color,
  ...
  float minutesAngle =
    float(uMinutes) * (2.0 * PI) / 60.0 +
    float(uSeconds) * (2.0 * PI) / 60.0 / 60.0;
  color = stroke(
    sdSegment(
      uv,
      vec2(0.0, 0.0),
      vec2(sin(minutesAngle), cos(minutesAngle)) * 0.65
    ),
    color,
    strokeColor,
    sh,
  ...
  color = fill(
    sdCircle(uv, 0.025 + th / 5.0 - mix(0.0, 0.06, mixAmount)),
    color,
    strokeColor,
    sh
  );
  color = fill(
    sdCircle(uv, 0.025 - mix(0.0, 0.04, mixAmount)),
    color,
    secondsStrokeColor,
    sh
  );
  ...
  float secondsAngle =
    float(uSeconds) * (2.0 * PI) / 60.0 +
    easeOutElastic(float(uMilliseconds) / 1000.0) * PI * (1.0 / 30.0);
  color = stroke(
    sdSegment(
      uv,
      vec2(sin(secondsAngle - PI), cos(secondsAngle - PI)) * 0.1,
      vec2(sin(secondsAngle), cos(secondsAngle)) * 0.8
    ),
    color,
    secondsStrokeColor,
    sh,
...
void main() {
  ...
  float normalizedAA =
    ((uAzimuthalAngle - uMinAzimuthAngle) /
      (uMaxAzimuthAngle - uMinAzimuthAngle) -
      0.5) *
    2.0;
  float normalizedPA =
    ((uPolarAngle - uMinPolarAngle) / (uMaxPolarAngle - uMinPolarAngle) - 0.5) *
    2.0;
